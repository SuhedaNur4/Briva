...
class UserContext:
    ...
    def from_dict(cls, data: dict) -> 'UserContext':
        return cls(city=_normalize_str(data.get('city', '')), interests=_normalize_list(data.get('interests', [])), skills=_normalize_list(data.get('skills', [])), available_days=_normalize_list(data.get('available_days', [])))
...
class RecommendationEngine:
    ...
    def explain(self, user: UserContext, event: 'Event') -> dict:
        scored = self.score_event(user, event)
        return {'event_id': scored.event_id, 'event_title': scored.event_title, 'total_score': scored.total_score, 'threshold': self.threshold, 'recommended': scored.total_score >= self.threshold, 'breakdown': scored.breakdown, 'details': scored.matching_details, 'scoring_rules': {'city_match': f'+{SCORE_CITY_MATCH} puan', 'interest_match': f'+{SCORE_INTEREST_MATCH} puan (her eşleşen alan için)', 'skill_match': f'+{SCORE_SKILL_MATCH} puan (her eşleşen beceri için)', 'day_match': f'+{SCORE_DAY_MATCH} puan'}}
